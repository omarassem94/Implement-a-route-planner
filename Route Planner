...
def get_current_node(self):
    ...
    for node in self.openSet:
        nodes[node]=self.calculate_fscore(node)        
...
def distance(self, node_1, node_2):
    ...
    L1 = self.map.intersections[node_1][0]-self.map.intersections[node_2][0]
    L2 = self.map.intersections[node_1][1]-self.map.intersections[node_2][1]
    D = (L1**2 + L2**2)**0.5  
    return D
...
def get_tentative_gScore(self, current, neighbor):
    ...
    return get_gScore(self,current) + distance(self, current, neighbor)
...
def calculate_fscore(self, node):
    ...
    # REMEMBER F = G + H
    return self.gScore.get(node,0.0) + self.distance(node,self.goal)
...
# 
# **ANSWER**: It is a method to determine the best path from one point(start) to another(goal), it does so by estimating the cost of travelling from point to point in terms of distance, time or any other relevant criteria and works on choosing the path with the minimum cost
# 

# ---
# How does A-Star search algorithm differ from Uniform cost search? What about Best First search?
# 
# **ANSWER**: the uniform cost search applies a method that depends only on the next path regardless of the complete picture and if the next path that is closer to the current now will lead us to the goal faster. i.e if the shortest available path is pointing in the opposite direction of the goal it will be discovered first although it will actually make us go further to the goal. best first search does quite the contrary, it relies mainly on how the next path is closer to the goal regardless to the cost of the path
# 

# ---
# What is a heuristic?
# 
# **ANSWER**: Heuristic is practically an estimate that is based on a lot of factors mainly previous experiences

# ---
# What is a consistent heuristic?
# 
# **ANSWER**: when the estimate is always less than or equal to the estimated distance from any neighbouring node to the goal, plus the cost of reaching that neighbour, the heuristic is then consistent

# ---
# What is a admissible heuristic? 
# 
# **ANSWER**: it is when the estimate is less than the actual
# 
